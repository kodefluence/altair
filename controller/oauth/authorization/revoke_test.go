...
func TestRevoke(t *testing.T) {
	...
	t.Run("Method", func(t *testing.T) {
		...
		assert.Equal(t, "POST", controller.Oauth().Authorization().Revoke(authorizationService).Method())
	})

	t.Run("Path", func(t *testing.T) {
		authorizationService := mock.NewMockAuthorization(mockCtrl)
		assert.Equal(t, "/oauth/authorizations/revoke", controller.Oauth().Authorization().Revoke(authorizationService).Path())
	})
	...
	t.Run("Control", func(t *testing.T) {
		t.Run("Given request with json body", func(t *testing.T) {
			t.Run("Return message with status 200", func(t *testing.T) {
				...
				ctrl := controller.Oauth().Authorization().Revoke(authorizationService)
